...
local handlerButton = CreateFrame("Button", "AdiBags_ConsolePortSupport_BindingHandlerButton", nil)
handlerButton:SetScript("OnClick", 
    ...
    function()
        mod:RegisterMessage("AdiBags_BagOpened", 
            function(event, bagName, bag)
                local frame = bag.frame
                SetOverrideBindingClick(frame, true, "PAD3", handlerButton:GetName())
            end
        )
